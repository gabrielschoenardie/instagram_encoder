...
function Invoke-FFmpeg {
    ...
    if ($ErrorLogFile) {
        try {
            # Se PowerShell 7+, use utf8NoBOM, senão UTF8 simples
            if ($PSVersionTable.PSVersion.Major -ge 7) {
                $stdErrBuilder.ToString() | Out-File -FilePath $ErrorLogFile -Encoding utf8NoBOM -Force
            } else {
    ...
    # Se tudo ocorreu bem:
    Write-Verbose "FFmpeg [$PassName] para [$CurrentFileName] concluído com sucesso."
    return $true
}
...
function Validate-Input {
    ...
    if ($UseCRF) {
        ...
        if ($PSBoundParameters.ContainsKey('Bitrate')) {
            # Aceita k, K, kbps, Kbps
            if ($Bitrate -notmatch '^\d+[kK](bps)?$') {
...
$lblTitle.Font     = New-Object System.Drawing.Font("Segoe UI",14)
...
$btnSelect.Font     = New-Object System.Drawing.Font("Segoe UI",9)
...
$gbSettings.Font     = New-Object System.Drawing.Font("Segoe UI",9)
...
$btnConvert.Add_Click({
    ...
    # Inicializa contador de sucesso
    $successCount = 0
